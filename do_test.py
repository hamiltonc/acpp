...
def test(binary: str, args: list[str]) -> int:
...
for binary, args_list in tests:
    ...
    print(
        ",".join(
            ["binary", "iterations", "connections"]
            + [f"run{i}" for i in range(0, runs)]
            + ["avg"]
        )
    )
    for args in args_list:
        ...
        print(
            ",".join(
                [binary] + args + [str(nsec) for nsec in runs_nsec] + [str(sum(runs_nsec) / runs)]
            )
        )
